...
void printEdges(std::vector<std::list<int>> *mstEdges) 
{ 
...
void primMST(Graph* graph, int* retArr) 
{ 
	...
	std::list<int> mstEdges[V];

	for(int i=0; i< V ; i++){
		if(parent[i]!=i) mstEdges[parent[i]].push_front(i);
	}

	std::list<int> printList;
	if(V>0){
		printList.push_front(0);
		int i=0;
		while( !printList.empty() ){
			int current = printList.front();
			printList.pop_front();
			retArr[i] = current; 
			//std::cout << current << std::endl;
			while(!mstEdges[current].empty()){
				printList.push_front(mstEdges[current].front());
				mstEdges[current].pop_front();
			}
			i++;
		}
	}
